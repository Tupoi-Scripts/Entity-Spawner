...
game.ReplicatedStorage.Bricks.Caption:FireClient(player, "Loaded Vynixu's Entity Spawner! Modded by Tupo")
wait(1)
game.ReplicatedStorage.Bricks.Caption:FireClient(player, "Removed 'require' thing. ")
...
local localChar = game.localPlayer.Character or game.localPlayer.CharacterAdded:Wait()
...
local floorReplicated = game.ReplicatedStorage:WaitForChild("FloorReplicated")
...
local moduleScripts = {
	Module_Events = game.ReplicatedStorage.ClientModules.Module_Events
	Main_Game = playerGui.MainUI.Initiator.Main_Game
	Earthquake = remotesFolder.RequestAsset:InvokeServer("Earthquake")
}
...
function FixRoomLights(room)
	...
	-- Set room ambient
	game.ReplicatedStorage.ClientModules.Module_Events.toggle(room, true, ambientStorage[room])
